 ...
 */
public class UserInfoPresenterTest implements UserInfoContract.UserInfoView {
    private GithubUser mGithubUserTest;
    private UserInfoContract.UserInfoPresenter mUserInfoPresenter;


    @Before
    public void setUp() throws Exception {
        mGithubUserTest = new GithubUser() {
            @Override
            public long id() {
                return 7405217;
            }
    ...
    public void onSuccess() throws Exception {
        UserInfoPresenter userInfoPresenter = Mockito.mock(UserInfoPresenter.class);

        mUserInfoPresenter.getUserInfo("Bigmercu");

        Mockito.verify(userInfoPresenter).onSuccess(mGithubUserTest);
